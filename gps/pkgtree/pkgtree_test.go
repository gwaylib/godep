...
import (
	...
	"github.com/gwaylib/godep/gps/paths"
	"github.com/gwaylib/godep/internal/fs"
	_ "github.com/gwaylib/godep/internal/test" // DO NOT REMOVE, allows go test ./... -update to work
)
...
func TestToReachMap(t *testing.T) {
	...
	allex := map[string][]string{
		b(""):               {"encoding/binary", "github.com/Masterminds/semver", "github.com/gwaylib/godep/gps", "go/parser", "hash", "net/http", "os", "sort"},
		b("m1p"):            {"github.com/gwaylib/godep/gps", "os", "sort"},
		b("namemismatch"):   {"github.com/Masterminds/semver", "os"},
		b("otherpath"):      {"github.com/gwaylib/godep/gps", "os", "sort"},
		b("simple"):         {"encoding/binary", "github.com/gwaylib/godep/gps", "go/parser", "hash", "os", "sort"},
		b("simple/another"): {"encoding/binary", "github.com/gwaylib/godep/gps", "hash", "os", "sort"},
	}
	...
	main = false
		...
		b("simple")+" encoding/binary",
		b("simple/another")+" encoding/binary",
		bl("otherpath", "m1p")+" github.com/gwaylib/godep/gps os sort",
	)
	validate()
	...
	ignore = []string{
		...
		bl("", "simple", "simple/another", "m1p", "otherpath")+" hash encoding/binary go/parser github.com/gwaylib/godep/gps sort",
		b("otherpath"),
		b("simple"),
	)
	validate()
	...
	ignore = append(ignore, b("namemismatch"))
		...
		// root pkg loses on everything in varied/simple/another and varied/m1p
		bl("", "simple", "simple/another", "m1p", "otherpath", "namemismatch")+" hash encoding/binary go/parser github.com/gwaylib/godep/gps sort os github.com/Masterminds/semver",
		b("otherpath"),
		b("simple"),
		b("namemismatch"),
	)
...
func TestFlattenReachMap(t *testing.T) {
	...
	for _, testCase := range []*flattenReachMapCase{
		...
		// we get github.com/gwaylib/godep/gps from m1p, too, so it should still be there
		{
			name:       "ignore simple",
		...
		// ignore two that should knock out gps
		{
			...
			expect:     except("sort", "github.com/gwaylib/godep/gps", "go/parser"),
			isStdLibFn: nil,
			main:       true,
			tests:      true,
			ignore: NewIgnoredRuleset([]string{
	...
	t.Run("disallowed", testFlattenReachMap(&ptree, &flattenReachMapCase{
		name:       "disallowed",
		expect:     []string{"github.com/gwaylib/godep/gps", "hash", "sort"},
		isStdLibFn: nil,
		main:       false,
		tests:      false,
	}))
