...
import (
	...
	"github.com/gwaylib/godep/gps/pkgtree"
...
func TestValidateUpdateArgs(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			args:      []string{"github.com/gwaylib/godep/cmd"},
			wantError: errUpdateArgsValidation,
			wantWarn: []string{
				"github.com/gwaylib/godep/cmd is not a project root, try github.com/gwaylib/godep instead",
			},
		},
		{
			name:      "not present in lock",
			args:      []string{"github.com/gwaylib/godep"},
			wantError: errUpdateArgsValidation,
			wantWarn: []string{
				"github.com/gwaylib/godep is not present in Gopkg.lock, cannot -update it",
			},
		},
		{
			name:      "cannot specify alternate sources",
			args:      []string{"github.com/gwaylib/godep:github.com/example/dep"},
			wantError: errUpdateArgsValidation,
			wantWarn: []string{
				"cannot specify alternate sources on -update (github.com/example/dep)",
			},
			lockedProjects: []string{"github.com/gwaylib/godep"},
		},
		{
			name:      "version constraint passed",
			args:      []string{"github.com/gwaylib/godep@master"},
			wantError: errUpdateArgsValidation,
			wantWarn: []string{
				"version constraint master passed for github.com/gwaylib/godep, but -update follows constraints declared in Gopkg.toml, not CLI arguments",
			},
			lockedProjects: []string{"github.com/gwaylib/godep"},
		},
		{
			name:      "flags after spec",
			args:      []string{"github.com/gwaylib/godep@master", "-v"},
			wantError: errUpdateArgsValidation,
			wantWarn: []string{
